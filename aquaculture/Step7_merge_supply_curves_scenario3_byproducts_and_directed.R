...
for(i in 1:length(species)){
  for(j in 1:length(cap_scens)){
    for(k in 1:length(catch_scalars)){
      
      # Params
      spp <- species[i]
      cap_scen <- cap_scens[j]
      scalar <- catch_scalars[k]
      scalar_format <- format(scalar, nsmall=2)
      print(paste(spp, cap_scen, scalar))
      ...
      infile <- paste0("AQ_finfish_species", spp, "_scenario3_whole_and_byproducts_", cap_scen, "_", scalar_format, "catch_scalar.Rdata")
      load(file.path(datadir, infile))
      ...
      wc_scurve1 <- wc_scurve %>% 
        ...
        rename(cap_scen=scenario, mt_yr_full=wc_mt_yr, meat_yr=wc_meat_yr_scen2) %>% 
        mutate(sector="Capture fisheries",
               mt_yr=mt_yr_full*(1-perc2reduction)*(1-perc2byprod),
               aq_spp=spp, 
               aq_cost_scalar=scalar,
               aq_scen="Scenario 3 - byproducts + directed") %>% 
      ...
      # Export
      outfile <- paste0("WC_AQ_supply_curve_merged_spp", spp, "_scenario3_byproducts_directed_", cap_scen, "_", scalar, "scalar.Rds")
      ...
      g <- ggplot() +
        ...
        ggtitle("Scenario 3: Feed from WC by-products\nand directed reduction fisheries") +
        scale_fill_manual(name="Protein source", values=alpha(c("blue", "darkgreen", "red"), 0.8)) +
        theme_bw() + my_theme
      g
      
      # Save
      outfig <- paste0("WC_AQ_supply_curve_merged_spp", spp, "_scenario3_byproducts_directed_", cap_scen, "_", scalar, "scalar.png")
