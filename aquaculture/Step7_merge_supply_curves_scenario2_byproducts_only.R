...
for(i in 1:length(species)){
  for(j in 1:length(cap_scens)){
    for(k in 1:length(catch_scalars)){
      ...
      infile <- paste0("AQ_finfish_species", spp, "_scenario2_byproducts_only_", cap_scen, "_", scalar_format, "catch_scalar.Rdata")
      load(file.path(datadir, infile))
      ...
      wc_scurve1 <- wc_scurve %>% 
        ...
        rename(cap_scen=scenario, mt_yr_full=wc_mt_yr, meat_yr=wc_meat_yr_scen1) %>% 
        mutate(mt_yr=mt_yr_full*(1-perc2byprod),
               sector="Capture fisheries", 
               aq_spp=spp, 
               aq_cost_scalar=scalar,
               aq_scen="Scenario 2 - byproducts only") %>% 
      ...
      # Export
      outfile <- paste0("WC_AQ_supply_curve_merged_spp", spp, "_scenario2_byproducts_", cap_scen, "_", scalar, "scalar.Rds")
      ...
      g <- ggplot() +
        ...
        ggtitle("Scenario 2: Feed from WC by-products only") +
        scale_fill_manual(name="Protein source", values=alpha(c("blue", "darkgreen", "red"), 0.8)) +
        theme_bw() + my_theme
      g
      
      # Save
      outfig <- paste0("WC_AQ_supply_curve_merged_spp", spp, "_scenario2_byproducts_", cap_scen, "_", scalar, "scalar.png")
