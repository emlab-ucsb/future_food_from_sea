...
# Build key
scenario_key <- expand.grid(spp_num=spp_num, cap_scen=cap_scen, cost_scalar=cost_scalars)


# Finfish only supply curve (constrained by scraps) - old version
#####################################################################################
    ...
    for(k in 1:length(cost_scalars)){
      ...
      f_scurve <- purrr::map_df(prices, function(x) {
        ...
        feed_avail_mt <- feed %>% 
          ...
          pull(paste0("feed_mt_yr_combo_spp", spp_do))
      ...
      # Plot fishmeal used
      g <- ggplot(f_scurve, aes(x=price, y=feed_mt_yr/1e6)) +
        geom_line() +
        labs(x="Price (USD/mt)", y="Feed used (millions mt)") +
        theme_bw()
      print(g)

      # Export results
      outfile <- paste0("AQ_finfish_species", spp_do, "_scenario3_whole_and_byproducts_",
...
#     # Price to evaluate and associated FM constraint
#     usd_mt <- p
#     feed_avail_mt <- feed$feed_mt_yr_combo[feed$scenario==cap_scen_do & feed$price==usd_mt]
#     
#     # Calculate and order by profits, cut off cells once feed is exceeded
#     results <- data %>%
#       # Identify cost and feed columns of species being evaluated
...
#   # Format supply curve
#   scurve_out <- f_scurve %>% 
#     mutate(cap_scen=cap_scen_do, aq_spp=spp_do, aq_scen="Scenario 3", aq_cost_scalar=cost_scalar_do) %>% 
...
# 
# # Plot results
# g <- ggplot(data, aes(x=tot_mt_yr, y=price, group=aq_cost_scalar, color=aq_cost_scalar)) +
#   geom_line() +
#   facet_grid(aq_spp ~ cap_scen) +
#   theme_bw()
# g
# 
# 
