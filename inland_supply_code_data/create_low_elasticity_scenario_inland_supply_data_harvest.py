...
def generate_inland_supply_data_low_elast_harvest():
    ...
    supply_dat['carp_aq'] = supply_dat.prices.apply(lambda x: supply_fcn(x, demand_multiplier=2267083.635, elasticity=0.3)) * (1 / carp_rat)
    supply_dat['carp_capt'] = supply_dat.prices.apply(lambda x: supply_fcn(x, demand_multiplier=60996.71167, elasticity=0.4)) * (1 / carp_rat)
    supply_dat['tilap_aq'] = supply_dat.prices.apply(lambda x: supply_fcn(x, demand_multiplier=2052769.746, elasticity=0.12)) * (1 / tilap_rat)
    supply_dat['tilap_capt'] = supply_dat.prices.apply(lambda x: supply_fcn(x, demand_multiplier=374295.5033, elasticity=0.09)) * (1 / tilap_rat)
    supply_dat['catfish_aq'] = supply_dat.prices.apply(lambda x: supply_fcn(x, demand_multiplier=67428.84794, elasticity=0.54)) * (1 / catfish_rat)
    supply_dat['catfish_capt'] = supply_dat.prices.apply(lambda x: supply_fcn(x, demand_multiplier=110734.9714, elasticity=0.14)) * (1 / catfish_rat)
    supply_dat['oth_aq'] = supply_dat.prices.apply(lambda x: supply_fcn(x, demand_multiplier=70093.17295, elasticity=0.64)) * (1 / oth_rat)
    supply_dat['oth_capt'] = supply_dat.prices.apply(lambda x: supply_fcn(x, demand_multiplier=674961.1013, elasticity=0.31)) * (1 / oth_capt_rat)
    ...
    #save data
    supply_dat.to_csv('compiled_data/inland_supply_dat_low_elast_harvest.csv')
